...
const TestimonialCard = ({ testimonial, onPlay }) => (
  <div className="flex-shrink-0 w-full md:w-[calc(50%-12px)] lg:w-[calc(33.333%-16px)]">
    <div className="bg-white rounded-lg overflow-hidden border border-[#f0f0f0] shadow-[0_2px_8px_rgba(0,0,0,0.06)] h-full flex flex-col">
      ...
      <div
        ...
        onClick={() =>
          testimonial.videoUrl && onPlay && onPlay(testimonial.videoUrl)
        }
        role={testimonial.videoUrl ? "button" : undefined}
        tabIndex={testimonial.videoUrl ? 0 : undefined}
        aria-label={
          testimonial.videoUrl
            ? `Play video for ${testimonial.names}`
            : undefined
        }
      >
        <img
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <div>
          ...
          <p className="text-[12px] text-[#7b7b7b]">{testimonial.location}</p>
        </div>
      </div>
    </div>
  </div>
...
const VideoModal = ({ videoUrl, onClose }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center">
    {/* Blur overlay */}
    <div
      className="absolute inset-0  bg-black/20 transition-opacity cursor-pointer"
      onClick={onClose}
    />
    {/* Close button outside modal, top-right of viewport */}
    <button
      className="fixed top-6 right-6 text-gray-500 hover:text-red-500 text-3xl font-bold z-[100] bg-white/80 rounded-full shadow-lg w-12 h-12 flex items-center justify-center"
      onClick={onClose}
    ...
    <div className="relative bg-white rounded-lg shadow-lg max-w-2xl w-full mx-4 p-0 flex flex-col items-center">
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-12 md:px-20 relative">
        <div className="text-center mb-10">
          ...
          <p className="text-[16px] text-[#7b7b7b] max-w-4xl mx-auto leading-relaxed">
            Every living room you see here was shaped by the rhythms of daily
            life—quiet mornings, lively evenings, and everything in between.
            These spaces reflect different ways of living, each one thoughtfully
            designed by the HomeLane team to feel just right! Let these living
            room interiors spark ideas for your own perfect home.
          </p>
        </div>
            ...
            >
              {slideItems.map((t, idx) => (
                <TestimonialCard
                  key={`${t.id}-${idx}`}
                  testimonial={t}
                  onPlay={(videoUrl) => setSelectedVideo(videoUrl)}
                />
              ))}
            </div>
          </div>
