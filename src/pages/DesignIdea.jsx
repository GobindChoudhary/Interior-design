    ...
    if (!categoryData) {
        return (
            <div className="bg-white min-h-screen py-20 text-center">
                ...
                <p className="text-gray-500 mb-8">The design idea category you are looking for doesn't exist.</p>
        ...
        <div className="bg-white min-h-screen py-10">
            ...
            <div className="max-w-7xl mx-auto px-4 md:px-8 pb-16">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-16">
                    {categoryData.blogs.map((blog) => (
                        <BlogCard key={blog.id} blog={blog} categoryId={currentCategorySlug} />
                    ))}
                </div>
            </div>
            ...
            {categoryData.blogs.length > 0 && (
                <div className="w-full flex justify-center pb-20">
                    <button className="border-2 border-[#e71c24] text-[#e71c24] font-semibold py-3 px-8 rounded hover:bg-[#e71c24] hover:text-white transition-colors duration-300">
                        LOAD MORE
                    </button>
                </div>
            )}
